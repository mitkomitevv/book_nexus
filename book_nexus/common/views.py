...
from book_nexus.books.models import Review, Rating, Comment, Book
...
class HomeView(UserReadingListMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.is_authenticated:
            ...
            user_ratings = Rating.objects.filter(user=self.request.user, book_id__in=book_ids).values('book_id', 'rating')

            user_rating_map = {rating['book_id']: rating['rating'] for rating in user_ratings}
