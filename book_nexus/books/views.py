...
from book_nexus.books.forms import (
    BookCreateForm,
    AuthorCreateForm,
    BookUpdateForm,
    ReviewForm,
...
class EditReviewView(LoginRequiredMixin, ModeratorsPermission, View):
    def test_func(self):
        ...
        return super().test_func() or self.request.user == review.user
...
class AddCommentView(LoginRequiredMixin, View):
    def post(self, request, review_pk):
        ...
        response_data = {
            ...
            "comment_created_at": timezone.localtime(comment.created_at).strftime(
                "%B %d, %Y %H:%M"
            ),
...
class EditCommentView(LoginRequiredMixin, ModeratorsPermission, UpdateView):
    ...
    def test_func(self):
        ...
        return super().test_func() or self.request.user == comment.user
    ...
    def render_to_response(self, context, **response_kwargs):
        if self.request.headers.get("x-requested-with") == "XMLHttpRequest":
            return JsonResponse(
                {"success": False, "errors": context["form"].errors}, status=400
            )
        else:
...
class ReviewCommentsModalView(View):
    def get(self, request, review_pk):
        try:
            review = Review.objects.select_related("user", "user__profile").get(
                pk=review_pk
            )
        except Review.DoesNotExist:
            raise Http404("No Review matches the given query.")

        comments = review.comments.select_related("user", "user__profile").order_by(
            "created_at"
        )
        ...
        if request.headers.get("x-requested-with") == "XMLHttpRequest":
            html = render_to_string(
                "reviews/modal_comments.html", context, request=request
            )
        ...
        try:
            ...
            if rating_value:
                ...
                _, _ = Rating.objects.update_or_create(
                    user=request.user, book=book, defaults={"rating": rating_value}
                )
    ...
    def delete(self, request, pk):
        try:
            ...
            if rating.exists():
                rating.delete()
                return JsonResponse(
                    {"success": True, "message": "Rating cleared successfully."}
                )
...
class AuthorDetailsView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["series_list"] = self.object.series.prefetch_related(
            "series_books__book"
        )
        return context
