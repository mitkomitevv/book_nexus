...
from book_nexus.accounts.mixins import RedirectAuthenticatedUserMixin
...
class UserRegistrationView(RedirectAuthenticatedUserMixin, CreateView):
    ...
    def form_invalid(self, form):
        messages.error(
            self.request,
            "There was an error creating your account. Please check the form below.",
        )
...
class UserEditView(LoginRequiredMixin, ModeratorsPermission, UpdateView):
    ...
    def test_func(self):
        ...
        return super().test_func() or self.request.user == user.user
    ...
    def get_queryset(self):
        ...
        if list_type == "want-to-read":
            queryset = queryset.filter(
                pk__in=WantToRead.objects.filter(user=user).values_list(
                    "book_id", flat=True
                )
            )
        elif list_type == "currently-reading":
            queryset = queryset.filter(
                pk__in=CurrentlyReading.objects.filter(user=user).values_list(
                    "book_id", flat=True
                )
            )
        elif list_type == "favorites":
            queryset = queryset.filter(
                pk__in=Favorites.objects.filter(user=user).values_list(
                    "book_id", flat=True
                )
            )
        else:
            queryset = queryset.filter(
                pk__in=Read.objects.filter(user=user).values_list("book_id", flat=True)
            )
