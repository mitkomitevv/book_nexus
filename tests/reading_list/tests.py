...
class ReadingListViewTests(TestCase):
    ...
    def setUpTestData(cls):
        cls.user = UserModel.objects.create_user(
            email="user@example.com", password="pass123", full_name="Test User"
        )
    ...
    def test_add_to_want_to_read_without_login(self):
        ...
        response = self.client.post(
            url, {"book_id": self.book.pk, "list_type": "want_to_read"}
        )
        ...
        self.assertFalse(
            WantToRead.objects.filter(user=self.user, book=self.book).exists()
        )
    ...
    def test_add_to_favorites_also_adds_to_read(self):
        ...
        self.client.post(
            add_url, {"book_id": self.book.pk, "list_type": "want_to_read"}
        )
        ...
        response = self.client.post(
            add_url, {"book_id": self.book.pk, "list_type": "favorites"}
        )
        data = response.json()
        ...
        self.assertTrue(
            Favorites.objects.filter(user=self.user, book=self.book).exists()
        )
    ...
    def test_remove_from_all_lists(self):
        ...
        self.assertFalse(
            CurrentlyReading.objects.filter(user=self.user, book=self.book).exists()
        )
        self.assertFalse(
            Favorites.objects.filter(user=self.user, book=self.book).exists()
        )
    ...
    def test_add_with_invalid_list_type(self):
        ...
        url = reverse("add-to-reading-list")
        response = self.client.post(
            url, {"book_id": self.book.pk, "list_type": "invalid_type"}
        )
