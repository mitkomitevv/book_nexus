...
class AccountsViewTests(TestCase):
    ...
    def test_user_registration_view(self):
        ...
        self.assertIn(
            "Your account has been created successfully.", str(messages_list[0])
        )
    ...
    def test_user_login_view(self):
        url = reverse("login")
        response = self.client.post(
            url, {"username": "normal@example.com", "password": "pass123"}
        )
    ...
    def test_user_edit_view(self):
        ...
        self.client.login(email="normal@example.com", password="pass123")
        response = self.client.post(
            url, {"full_name": "Updated Name", "country": "Newland"}
        )

        self.assertRedirects(
            response, reverse("profile_details", kwargs={"pk": self.user.pk})
        )
    ...
    def test_follow_unfollow_user(self):
        ...
        self.assertRedirects(
            response, reverse("profile_details", kwargs={"pk": other_user.pk})
        )
        self.assertTrue(
            Follow.objects.filter(follower=self.user, followed_user=other_user).exists()
        )
