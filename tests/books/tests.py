...
class ViewsTestsWithCustomUser(TestCase):
    def setUp(self):
        ...
        self.review = Review.objects.create(
            book=self.book, user=self.user, content="Great book!"
        )
    ...
    def test_add_review_view(self):
        ...
        self.assertRedirects(
            response, reverse("book-details", kwargs={"pk": self.book.pk})
        )
        ...
        self.assertTrue(
            Review.objects.filter(
                book=self.book, user=self.user, content="Another review!"
            ).exists()
        )
    ...
    def test_edit_review_view_as_moderator(self):
        ...
        response = self.client.post(
            url, {"content": "Updated content!"}, HTTP_X_REQUESTED_WITH="XMLHttpRequest"
        )
    ...
    def test_add_comment_view(self):
        ...
        response = self.client.post(
            url, {"content": "Nice comment!"}, HTTP_X_REQUESTED_WITH="XMLHttpRequest"
        )
        ...
        self.assertTrue(
            Comment.objects.filter(
                review=self.review, user=self.user, content="Nice comment!"
            ).exists()
        )
